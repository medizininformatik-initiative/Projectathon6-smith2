...
if (filterConsent) {
  consent_list <- lapply(list, function(x) {
    ...
    consent_request <- fhir_url(
      url = base,
      resource = "Consent",
      parameters = c(patient = ids)
    )
    
    consent_bundles <- fhir_search(
      consent_request,
      username = username,
      password = password,
      token = token,
  ...
  consent_bundles <-
    fhircrackr:::fhir_bundle_list(unlist(consent_list, recursive = F))
  fhir_save(bundles = consent_bundles, directory = "Bundles/Consents")
  
  consent_description <- fhir_table_description(
    "Consent",
    cols = c(
  ...
  consent_table <-
    fhir_melt(
      consent_table,
      columns = c("provision.code", "provision.system"),
      brackets = brackets,
      sep = sep,
      all_columns = T
    )
  ...
  consent_table <-
    fhir_rm_indices(consent_table, brackets = brackets)
  
  #find Patients that have code for MDAT_wissenschaftlich_nutzen_EU_DSGVO_NIVEAU
  allowed_pats <-
    consent_table[provision.code == "2.16.840.1.113883.3.1937.777.24.5.3.8" &
                    provision.system == "urn:oid:2.16.840.1.113883.3.1937.777.24.5.3"]
